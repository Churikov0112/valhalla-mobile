...
Java_com_valhalla_valhalla_ValhallaKotlin_height(JNIEnv *env,
    ...
    try {
        ValhallaActor valhallaActor(config_path);
        result = valhallaActor.height(request);
    } catch (const valhalla::valhalla_exception_t &err) {
        printf("[ValhallaActor] height valhalla_exception: %s\n", err.what());
        std::string code = std::to_string(err.code);
        std::string message = err.message.c_str();

        result = "{\"code\":" + code + ",\"message\":\"" + message + "\"}";
    } catch (const std::exception &err) {
        printf("[ValhallaActor] height std::exception: %s\n", err.what());
        result = "{\"code\":-1,\"message\":\"" + std::string(err.what()) + "\"}";
    } catch (...) {
        printf("[ValhallaActor] height unknown exception");
        result = "{\"code\":-1,\"message\":\"unknown exception\"}";
    }
...
Java_com_valhalla_valhalla_ValhallaKotlin_traceRoute(JNIEnv *env,
    ...
    try {
        ValhallaActor valhallaActor(config_path);
        result = valhallaActor.trace_route(request);
    } catch (const valhalla::valhalla_exception_t &err) {
...
Java_com_valhalla_valhalla_ValhallaKotlin_matrix(JNIEnv *env,
    ...
    try {
        ValhallaActor valhallaActor(config_path);
        result = valhallaActor.matrix(request);
    } catch (const valhalla::valhalla_exception_t &err) {
        printf("[ValhallaActor] matrix valhalla_exception: %s\n", err.what());
        std::string code = std::to_string(err.code);
        std::string message = err.message.c_str();

        result = "{\"code\":" + code + ",\"message\":\"" + message + "\"}";
    } catch (const std::exception &err) {
        printf("[ValhallaActor] matrix std::exception: %s\n", err.what());
        result = "{\"code\":-1,\"message\":\"" + std::string(err.what()) + "\"}";
    } catch (...) {
        printf("[ValhallaActor] matrix unknown exception");
        result = "{\"code\":-1,\"message\":\"unknown exception\"}";
    }
...
JNICALL
Java_com_valhalla_valhalla_ValhallaKotlin_isochrone(JNIEnv *env,
                                                     jobject thiz,
                                                     jstring jRequest,
                                                     jstring jConfigPath) {
    ...
    try {
        ValhallaActor valhallaActor(config_path);
        result = valhallaActor.isochrone(request);
    } catch (const valhalla::valhalla_exception_t &err) {
        printf("[ValhallaActor] isochrone valhalla_exception: %s\n", err.what());
        std::string code = std::to_string(err.code);
        std::string message = err.message.c_str();

        result = "{\"code\":" + code + ",\"message\":\"" + message + "\"}";
    } catch (const std::exception &err) {
        printf("[ValhallaActor] isochrone std::exception: %s\n", err.what());
        result = "{\"code\":-1,\"message\":\"" + std::string(err.what()) + "\"}";
    } catch (...) {
        printf("[ValhallaActor] isochrone unknown exception");
        result = "{\"code\":-1,\"message\":\"unknown exception\"}";
    }
...
std::string height(const char *request, void* actor) {
    std::string result;
    try {
        result = ((ValhallaActor*) actor)->height(request);
    } catch (const valhalla::valhalla_exception_t &err) {
        printf("[ValhallaActor] height valhalla_exception: %s\n", err.what());
        std::string code = std::to_string(err.code);
        std::string message = err.message.c_str();
...
std::string matrix(const char *request, void* actor) {
    std::string result;
    try {
        result = ((ValhallaActor*) actor)->matrix(request);
    } catch (const valhalla::valhalla_exception_t &err) {
        printf("[ValhallaActor] matrix valhalla_exception: %s\n", err.what());
        std::string code = std::to_string(err.code);
        std::string message = err.message.c_str();
...
std::string isochrone(const char *request, void* actor) {
    std::string result;
    try {
        result = ((ValhallaActor*) actor)->isochrone(request);
    } catch (const valhalla::valhalla_exception_t &err) {
        printf("[ValhallaActor] isochrone valhalla_exception: %s\n", err.what());
        std::string code = std::to_string(err.code);
        std::string message = err.message.c_str();
